...
#   - HTTP direct (services .minint.fr et cluster internes si disponibles)
# ---------------------------------------------------------------------------
set -euo pipefail
...
# Image avec curl ET wget, mirror sur DockerHub etiquet (compte authentifie
# via regcred → pas de rate limit).
IMAGE="${TEST_IMAGE:-docker.io/etiquet/curl:8.10.1}"
...
#   direct  = direct, sans proxy (domaines internes .minint.fr)
...
ENDPOINTS=(
  ...
  "DockerHub image manifest|https://index.docker.io/v2/etiquet/curl/manifests/8.10.1|proxy"

  "LLM API gpu.minint.fr|https://<LLM_API_HOSTNAME>/v1/models|direct"
...
SCRIPT_HEADER
  ...
  echo 'echo ""'
  echo 'echo "--- Endpoints direct (.minint.fr) ---"'
  for entry in "${ENDPOINTS[@]}"; do
    IFS='|' read -r name url type <<< "$entry"
