...
#
# Prerequisites: kubectl, helm, docker login to DockerHub (etiquet)
...
NS="dgx-e2e-test"
IMAGE="${DM_IMAGE:-docker.io/etiquet/device-management:0.5.16-waf-fix}"
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: nginx
          image: docker.io/etiquet/nginx:1.27-alpine
...
spec:
  ...
  template:
    ...
    spec:
      imagePullSecrets: [{ name: regcred }]
      initContainers:
        - { name: w, image: docker.io/etiquet/postgres:16-alpine, command: ["sh","-c","until pg_isready -h postgres; do sleep 1; done"] }
      containers:
...
kubectl -n "$NS" run e2e-runner --restart=Never \
  --image=docker.io/etiquet/curl:8.10.1 \
