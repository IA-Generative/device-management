...
```json
GET https://<SCALEWAY_HOSTNAME>/catalog/api/plugins
{
  "plugins": [{
    "slug": "mirai-libreoffice",
    "name": "MIrAI — IA'ssistant LibreOffice",
    "intent": "Assister les redacteurs...",
    "icon_url": "https://<SCALEWAY_HOSTNAME>/catalog/api/plugins/mirai-libreoffice/icon.png",
...
3. URL : `https://<SCALEWAY_HOSTNAME>/catalog`
4. Hauteur : 80vh
...
def catalog_index(request):
    try:
        resp = requests.get(
            "https://<SCALEWAY_HOSTNAME>/catalog/api/plugins",
            timeout=5,
        )
...
def catalog_detail(request, slug):
    try:
        resp = requests.get(
            f"https://<SCALEWAY_HOSTNAME>/catalog/api/plugins/{slug}",
            timeout=5,
        )
        resp.raise_for_status()
        plugin = resp.json()
