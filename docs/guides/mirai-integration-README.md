...
- Les pages publiques du catalogue DM doivent etre deployees et accessibles :
  - `GET https://<SCALEWAY_HOSTNAME>/catalog/api/plugins` (API JSON)
  - `GET https://<SCALEWAY_HOSTNAME>/catalog/{slug}` (page de detail)
  - `GET https://<SCALEWAY_HOSTNAME>/catalog/{slug}/download` (telechargement)
