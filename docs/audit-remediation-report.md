...
| **VULN-003** | Révocation relay absente (Élevée 8,1) | 🟡 **Partiel** | Vérif `revoked` déjà active (`main.py:1317`) + rotation pose `revoked_at` (`:1281`). **Reste : endpoint admin de révocation explicite + UI (CT-1).** ⏳ |
