...
Le node hôte du DGX (`kubernetes.gpu.minint.fr`, single-node k3s) a une
...
seul egress disponible est un proxy HTTP (`10.234.6.19:3128`) qui ne
...
- soit un serveur NTP interne dans `10.0.0.0/8` ou `.minint.fr` ajouté
...
```bash
LOCAL=$(date -u +%s)
REMOTE=$(date -u -d "$(curl -sk -I https://kubernetes.gpu.minint.fr | grep -i '^date:' | sed 's/^[Dd]ate: //' | tr -d '\r')" +%s)
