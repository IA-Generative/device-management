...
if [ ! -f "$REPO_ROOT/deploy-dgx/db-schema.sql" ]; then
  echo "Missing $REPO_ROOT/deploy-dgx/db-schema.sql (build context must be repo root)." >&2
  exit 1
fi
...
docker build -f "$REPO_ROOT/deploy-dgx/Dockerfile" -t "$IMAGE_REF" "$REPO_ROOT"
...
docker buildx build \
  ...
  -f "$REPO_ROOT/deploy-dgx/Dockerfile" \
  -t "$IMAGE_REF" \
  "$REPO_ROOT" \
  --push
